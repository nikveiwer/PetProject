...
const AnimalContent: React.FC<Props> = ({ animalId }) => {
    ...
    const { getPets, status, setStatus, _tranformToPetCard, _tranformToPetInformation } =
        PetsFetching();

    const [singleAnimal, setSingleAnimal] = useState<IPetInformation>(
        null as any as IPetInformation,
    );
    const [likedAnimalInformation, setLikedAnimalInformation] = useState<IPetCard>(
        null as any as IPetCard,
    );

    const getSinglePet = async (animalId: number, accessToken: string) => {
        const data = (await getPets('animals', '', accessToken, [String(animalId)])) as unknown;
                ...
                return (
                    <>
                        ...
                        <section className=" my-7 px-2 flex lg:flex-row justify-center flex-col gap-5 items-start">
                            ...
                            <AnimalInquiry {...likedAnimalInformation}></AnimalInquiry>
                        </section>
                    </>
                );
            } else {
