...
export const PetsFetching = () => {
    ...
    const getPets = async (category: string = "animals", action: string = "", accsessToken: string, ...paramsAndValues: string[][]): Promise<unknown> => {
        ...
        if (paramsAndValues[0].length === 1) {
            ...
            paramsPath = paramsAndValues.map(item => {
                return item.join("=")
            }).join("&");
